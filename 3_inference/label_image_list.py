...
# def extr_pointidyear(sss):
#     pointidyear = sss.split('/')[7].replace('.JPG', '')
#     return(pointidyear)

def extr_pointidyear(sss):
    year = sss.split('/')[8].replace('LUCAS', '')
    pointid = sss.split('/')[12].split('_')[1]
    pointidyear = pointid + year
    return(pointidyear)
...
if __name__ == "__main__":
  ...
  df = pd.read_csv(image_list, header = None)
  df = df.rename(columns={0: "Files"})
  #df.head()
...
df['pointidyear'] = ''
df['pointidyear'] = df.apply(lambda row: extr_pointidyear(str(row['files'])), axis = 1)
#df['pointidyear'] = pd.to_numeric(df['pointidyear'], errors='coerce').astype('Int64')
...
gt['pointidyear'] = gt['pointidyear'].astype(str)
...
df_join = pd.merge(df, gt, how='left', on="pointidyear")

#print(df_join.head())

#to erase
out_file = os.path.join(output_dir,"cnn_output_data_check.csv")
#df_join.drop(columns=['files'], axis=1, inplace=True)
